...
import { fetchMyLectures} from '../../../server-mocks/utils';
import { LecturePreview, Person } from '../../../types';
import MyLectures from '../index';
...
const NewLectureDialog = ({ open, onClose, setLectures }: NewLectureDialogProps) => {
  ...
  const onSubmit: SubmitHandler<Inputs> = data => {
    const newLecture: LecturePreview = {...data,
      id: (MyLectures.length + lectures.length + 1).toString(),
    ...
    setLectures(JSON.stringify([...fetchMyLectures(), newLecture]));
    onClose();
  }
